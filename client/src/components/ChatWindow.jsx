...
const ChatWindow = ({ projectId, isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 w-96 card-strong rounded-xl z-50 flex flex-col transition-all duration-300 ${
      isMinimized ? 'h-14' : 'h-[500px]'
    }`}>
      {/* Header */}
      <div className="px-4 py-3 bg-accent text-white rounded-t-xl flex items-center justify-between cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
      ...
      {!isMinimized && (
        <>
                    ...
                    <div className={`max-w-[75%] ${isOwnMessage ? 'order-2' : 'order-1'}`}>
                      ...
                      <div className={`px-4 py-2 rounded-2xl ${
                        isOwnMessage 
                          ...
                          : 'bg-paper text-ink rounded-bl-none shadow-sm'
                      }`}>
          ...
          {/* Input */}
          <form onSubmit={handleSendMessage} className="p-4 bg-paper border-t border-line rounded-b-xl">
            <div className="flex items-center space-x-2">
              <input
                ref={inputRef}
                type="text"
                ...
                className="flex-1 px-4 py-2 border border-line rounded-full focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent text-sm bg-white"
              />
              <button
                type="submit"
                disabled={!newMessage.trim()}
                className="p-2 btn btn-primary rounded-full disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
